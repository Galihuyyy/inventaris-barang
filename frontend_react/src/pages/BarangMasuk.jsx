...
const BarangMasuk = () => {
    ...
    function createBarangMasuk () {
        axios.post(`${apiUrl}/barang-masuk`, formCreate, {
            headers: {
            Authorization: `Bearer ${userToken}`,
            },
        })
    ...
    function deleteBarangMasuk () {
        axios.delete(`${apiUrl}/barang-masuk/${selectedId}`, {headers : {Authorization : `Bearer ${userToken}`}})
        .then(res => {
            setMessage(res.data.message)
            setTimeout(function() {
                setMessage('')
  ...
  return (
    <PageLayout>
        ...
        <header><h1 className='text-3xl font-semibold text-neutral-700'>Barang Masuk</h1></header>
        <div className="w-full overflow-x-scroll py-4">
            <button onClick={() => {toggleModalCreate()}} className=' px-2 py-1 mb-3 bg-blue-500 float-end text-white rounded-md'><i className="fa fa-plus mr-3"></i>Tambah Barang Masuk</button>
            <table className="w-full">
                <thead className='h-12 text-left text-neutral-700 bg-gray-300 border-b'>
                    <tr>
                        ...
                        <th>Tanggal</th>
                        <th>Barang</th>
                        <th className='w-40'>Jumlah Masuk</th>
        ...
        {showModalCreate &&
            <Modal title={'tambah barang masuk'}>
            <div className='mb-5'>
        ...
        {showModalDelete && 
            <Modal title={'hapus data'}>
                ...
                <button onClick={() => {deleteBarangMasuk()}} className='px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700'>Hapus</button>
            </Modal>
        }
    </PageLayout>
  )
