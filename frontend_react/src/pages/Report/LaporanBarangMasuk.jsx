...
const LaporanBarangMasuk = () => {
    ...
    function getLaporanBarangMasuk(filter) {
        axios.get(`${apiUrl}/laporan-barang-masuk?tanggal_mulai=${filter.tanggal_mulai}&tanggal_akhir=${filter.tanggal_akhir}`, {
            headers: {
                Authorization: `Bearer ${userToken}`
            }
        })
        .then(res => {
            setLaporanBarangMasuk(res.data.data)
            setIsSubmitted(true)
        })
        .catch(err => {
            console.log(err)
    ...
    return (
        <PageLayout>
            <header>
                <h1 className='text-3xl font-semibold text-neutral-700 mb-12'>Laporan Barang Masuk</h1>
            </header>
            ...
            {isSubmitted && (
                <div className="w-full overflow-x-scroll py-4 bg-white rounded-xl px-6" ref={contentRef}>
                    <h1 className='mb-3'>
                        ...
                        <span className='ml-2 text-neutral-600'> Laporan Barang Masuk Tanggal <b>{filter.tanggal_mulai}</b> s/d <b>{filter.tanggal_akhir}</b></span>
                    </h1>
                    <table className="w-full">
                        ...
                        <tbody className='text-start text-neutral-700'>
                            {laporanBarangMasuk.map((item, index) => (
